...
D=$(new_dir flat)
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"hello world","max_iterations":7,"completion_promise":"<promise>DONE</promise>"}}}'
...
D=$(new_dir nested)
inner='{\"methodParams\":{\"prompt\":\"nested ok\",\"max_iterations\":3,\"completion_promise\":\"<promise>NESTED</promise>\"}}'
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"only prompt"}}}'
out=$(run_hook "$input" "$D")
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"alt key","iterations":12}}}'
out=$(run_hook "$input" "$D")
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"no promise","max_iterations":2}}}'
out=$(run_hook "$input" "$D")
...
input='{"workspace":"'"$D"'","tool_input":{"params":{"prompt":"via workspace","max_iterations":1}}}'
out=$(run_hook "$input" "$D")
...
pushd "$D" > /dev/null
input='{"tool_input":{"params":{"prompt":"pwd default","max_iterations":4}}}'
...
pushd "$D" > /dev/null
input='{"cwd":"/this/path/does/not/exist","tool_input":{"params":{"prompt":"bad cwd","max_iterations":1}}}'
echo "$input" | "$SCRIPT" > /dev/null 2>&1
...
input='{"cwd":"'"$D"'","tool_input":{"unrelated":"x"}}'
echo "$input" | "$SCRIPT" > /dev/null 2>&1
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"with \"quotes\" inside","max_iterations":1}}}'
out=$(run_hook "$input" "$D")
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"line1\nline2","max_iterations":1}}}'
echo "$input" | "$SCRIPT" > /dev/null 2>&1
...
input='{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"x","max_iterations":1,"completion_promise":"RAW_DONE"}}}'
out=$(run_hook "$input" "$D")
...
input=$'{"cwd":"'"$D"'","tool_input":{"params":{"prompt":"ansi here\x1b[31m","max_iterations":1}}}'
out=$(run_hook "$input" "$D")
...
input='{"project_dir":"'"$D"'","tool_input":{"params":{"prompt":"via project_dir","max_iterations":1}}}'
out=$(run_hook "$input" "$D")
...
input='{"transcript_cwd":"'"$D"'","tool_input":{"params":{"prompt":"via transcript","max_iterations":1}}}'
out=$(run_hook "$input" "$D")
check "transcript_cwd → init"  "Agent-loop initialized" "$out"
check "transcript_cwd → state" "via transcript"         "$out"
rm -rf "$D"

# 17. empty stdin produces no state file, exits 0
out=$(echo "" | "$SCRIPT" 2>&1)
rc=$?
